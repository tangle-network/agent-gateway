...
describe('A2A payment ownership races', () => {
  ...
  it('recovers generic MPP usage acknowledgement loss from the durable task receipt', async () => {
    ...
    const recoveredTask = await taskStore.get('task-stripe-usage-recovery')
    expect(recoveredTask?.metadata?.gatewayOrigin).toBeDefined()
    expect(recoveredTask?.metadata?.gatewayFinalizing).toBeUndefined()
    expect(recoveredTask?.metadata?.gatewayPaymentRecovery).toBeUndefined()
  })
})
